...
void Explore() {
  queue<Point> Q;
  seen[luka.r][luka.c].insert(luka);
  Q.push(luka);
  while (!Q.empty() && (int)seen[luka.r][luka.c].size() < R + C) {
    Point s = Q.front();
    Q.pop();
    for (int d = 0; d < 4; ++d) {
...
void Analyze() {
  if (seen[luka.r][luka.c].size() > 1) {
    set<Point> deltas;

    set<Point> const& S = seen[luka.r][luka.c];
...
bool Solve(Point const& p) {
  ...
  if (pattern_type == NONE) {
    ...
    while (hi - lo > 10) {
      ...
      int64 dist1 = abs(luka.r - t1.r) + abs(luka.c - t1.c);
      int64 dist2 = abs(luka.r - t2.r) + abs(luka.c - t2.c);
