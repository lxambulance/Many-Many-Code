...
int be_now;
char be_buf[100*1024];

void new_int(b_element **b,long long x){
	*b=new b_element;
	(*b)->type=0;
	(*b)->x=x;
	(*b)->s=NULL;
...
void new_str(b_element **b,char *s,int len){
	*b=new b_element;
	(*b)->type=1;
	(*b)->x=len;
	(*b)->s=new char[len];
...
void lis_add(b_element *b0,b_element *b1){
	if (b0->head==NULL)
		b0->head=b1;
	else {
		b_element *tmp=b0->head;
		while (tmp->next!=NULL) tmp=tmp->next;
		tmp->next=b1;
	}
}

void dic_add(b_element *b0,b_element *b1,b_element *b2){
	if (b0->head==NULL)
		b0->head=b1,b1->next=b2;
	else {
		b_element *tmp=b0->head;
		while (tmp->next!=NULL) tmp=tmp->next;
		tmp->next=b1,b1->next=b2;
	}
}
...
void b_element_show_int(long long x){
	if (x==0) {
		be_buf[be_now++]='0';
		return;
	}
	...
	while (len--) be_buf[be_now++]=s[len];
}

void b_element_show(b_element *b){
	if (b==NULL) return;
	if (b->type==0) {
		be_buf[be_now++]='i';
		b_element_show_int(b->x);
		be_buf[be_now++]='e';
	}
	else if (b->type==1) {
		b_element_show_int(b->x);
		be_buf[be_now++]=':';
		memcpy(be_buf+be_now,b->s,b->x);
		be_now+=b->x;
		delete[] b->s;
	}
	else if (b->type==2) {
		be_buf[be_now++]='l';
		b_element_show(b->head);
		be_buf[be_now++]='e';
	}
	else if (b->type==3) {
		be_buf[be_now++]='d';
	...
	b_element_show(b->next);
	delete b;
}

void b_element_show_notdel(b_element *b){
	if (b==NULL) return;
	if (b->type==0) {
		be_buf[be_now++]='i';
		b_element_show_int(b->x);
