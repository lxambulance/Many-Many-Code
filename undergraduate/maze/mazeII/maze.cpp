...
void Maze::SFMLshow(){
    int tot=0;
    sf::RectangleShape *p=new sf::RectangleShape[15000];
    p[tot].setPosition(3.0,3.0);
    p[tot].setSize(sf::Vector2f(m*12-3,n*12-3));
    p[tot].setOutlineThickness(3);
    p[tot].setOutlineColor(sf::Color::Blue);
    tot++;
    for (int i=0;i<n;++i)
        for (int j=0;j<m;++j)
        if (b[0][0]&&b[i][j]) {
            p[tot].setPosition(j*12+3,i*12+3);
            p[tot].setSize(sf::Vector2f(9,9));
            p[tot].setFillColor(sf::Color::Yellow);
            tot++;
        }
    for (int i=0;i<n;++i)
        for (int j=0;j<m;++j) {
            if (!a[i][j][2]) {
                p[tot].setPosition(j*12,i*12);
                p[tot].setSize(sf::Vector2f(15,3));
                p[tot].setFillColor(sf::Color::Blue);
                tot++;
            }
            if (!a[i][j][3]) {
                p[tot].setPosition(j*12,i*12);
                p[tot].setSize(sf::Vector2f(3,15));
                p[tot].setFillColor(sf::Color::Blue);
                tot++;
            }
        }
    sf::RenderWindow *window=new sf::RenderWindow(sf::VideoMode(m*12+3,n*12+3), "LXmaze!(Please shutdown after use)");
    while (window->isOpen()) {
        sf::Event event;
        ...
        window->clear();
        for (int i=0;i<tot;++i) window->draw(p[i]);
        window->display();
    }
    delete window;
    delete[] p;
...
int Maze::loadMapFrom(char x[]){
	...
	fscanf(fp,"%d",&p);
	for (int i=0;i<p;++i) {
		fscanf(fp,"%d%d%d%d%d",&t,&x1,&y1,&x2,&y2);//0ºá1Êú
		if (t==0) {
