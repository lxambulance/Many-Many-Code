...
struct P{
	...
	P operator*(const P p) {return P(y*p.z-z*p.y,z*p.x-x*p.z,x*p.y-y*p.x);}//叉积
	double operator^(const P p) {return x*p.x+y*p.y+z*p.z;}//点积
};
P dd;
struct fac{
...
struct T3dhull{
	...
	double dist(P a){return sqrt(a.x*a.x+a.y*a.y+a.z*a.z);}//两点长度
	double area(P a,P b,P c){return dist((b-a)*(c-a));}//三角形面积*2
	double volume(P a,P b,P c,P d){return (b-a)*(c-a)^(d-a);}//四面体有向体积*6
	double ptoplane(P &p,fac &f){//正：点在面同向
		 P m=ply[f.b]-ply[f.a],n=ply[f.c]-ply[f.a],t=p-ply[f.a];
		 return (m*n)^t;
	}
...
int main(){
	...
	while(scanf("%d",&hull.n)!=EOF){
		if(hull.n==0) break;
		int i,j,q;
		for(i=0;i<hull.n;i++) scanf("%lf%lf%lf",&hull.ply[i].x,&hull.ply[i].y,&hull.ply[i].z);
		hull.construct();
		scanf("%d",&q);
		for(j=0;j<q;j++) {
