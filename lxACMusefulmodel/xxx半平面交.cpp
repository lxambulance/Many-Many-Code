...
void HalfPlaneIntersect(){
	...
	sort(l,l+n,cmp);//极角排序
	for (i=0,j=0;i<n;i++)
	if (dblcmp(l[i].angle-l[j].angle)>0) l[++j]=l[i];//排除极角相同（从了l[1]开始比较）
	n=j+1;//个数
	dq[0]=0; dq[1]=1;
	top=1; bot=0;
	for (i=2;i<n;i++){
	...
	dq[++top]=dq[bot];
	for (pn=0,i=bot;i<top;i++,pn++) getIntersect(l[dq[i+1]],l[dq[i]],p[pn]);//更新重复利用p数组
}
double getArea(){
	if (pn<3) return 0;
	double area=0;
	for (int i=1;i<pn-1;i++) area+=multi(p[0],p[i],p[i+1]);//利用p数组求面积
	return fabs(area)/2;
}
int main(){
	int t,i;
	...
	while (t--){
		...
		for (i=0;i<n-1;i++) addLine(l[i],p[i].x,p[i].y,p[i+1].x,p[i+1].y);
